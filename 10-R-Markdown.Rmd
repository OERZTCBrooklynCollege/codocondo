...
## Learning Objectives {r-markdown-objectives}
...
## Creating an R Markdown File {#r-markdown-creation}
...
## Parts of an R Markdown File {#r-markdown-parts}
...
Take the code below. This, as well as *all* of the code in the book is written inside R chunks. Below is some code that is from all the way back from section \@ref(tidyverse-together) of the book.
...
Let's go back to that `penguins_new` dataset. Before in section \@ref(code-chunks), we "created" it. Now, let's see what happens when we call the output.
...
This visualization, which we originally built in Section \@ref(ggplot-scatterplot), not only displays the data, but provides a caption to explain the visualization, all thanks to `fig.cap`.

Yes, a picture is worth 1,000 words, but sometimes it can be helpful to add some words to help *guide* viewers.

### Sections {#r-markdown-sections}
...
## Knitting an R Markdown File {#knitting}
...
## Publishing an R Markdown file {#publishing}
...
### Checklists {#r-markdown-checklist-creation}
...
### Standout sections {#r-markdown-standouts}
...
### Changing Setting of Specific R Chunks {#r-markdown-settings}
...
## Key Takeaways {#r-markdown-takeaways}
...
## Checklist {#r-markdown-checklist}
...
## Key Functions & Commands {#r-markdown-keyfunctions}
...
## Summary of Common R Markdown Syntax {#r-markdown-syntax-summary}
