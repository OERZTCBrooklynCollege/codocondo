...
## Introduction {#intro-cor}

In everyday communication, the word **correlation** is thrown around a lot, but, what does it actually mean? 
In this lesson, we will explore how to use **correlations** to better understand how our variables interact with each other.
...
### Learning Objectives {#cor-objectives}
...
We start by importing the necessary packages, reading in our Excel file, and quickly doing an overview of the data. For a quick review on loading data into R, refer back to section \@ref(intro-reading-data).
...
## Cleaning our data {#cleaning-data-correlation}
...
We could use the `rename()` command from the `dplyr` package like in Section \@ref(tidyverse-rename), but there is more than one column, so we can streamline this process. Using the `clean_names()` function from the `janitor` package, we can easily remove the spaces in our column names
...
## Visualizing Relationships {#cor-viz}
...
```{r Anxiety-Performance,fig.cap = "Scatterplot illustrating the relationship between exam anxiety and exam performance, with a fitted linear trend line. The downward trend indicates a negative linear association, suggesting that higher anxiety levels are associated with lower exam scores. This visualization supports the use of correlation to formally assess the direction and strength of the relationship."}
...
```{r Pairs,fig.cap = "Scatterplot matrix displaying pairwise relationships among studying time, exam performance, and exam anxiety. Each panel shows the relationship between two variables, allowing for simultaneous assessment of direction, strength, and linearity prior to formal correlation analysis.", message=FALSE, warning=FALSE}
...
## Running Correlations (r) {#running-cor}
...
```{r Strength-Guidelines, echo=FALSE, message=FALSE, warning=FALSE}
library(knitr)
library(kableExtra)
...
## Partial Correlations {#partial-cor}
...
```{r ppcor-test, message=FALSE, warning=FALSE}
library(ppcor)
...
## Biserial and Point-Biserial Correlations {#biserial-cor}
...
## Grouped Correlations {#grouped-cor}
...
In chapter (\@ref(linear-reg)), we’ll use what we learned here to build predictive models — moving from describing relationships to forecasting outcomes.

## Conclusion {#cor-conclusion}
...
## Key Functions & Commands {#cor-keyfunctions}
...
## Example APA-style Write-up {#cor-apa}
