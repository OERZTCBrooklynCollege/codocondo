...
```{r AnscombeData, message=FALSE, warning=FALSE}
library(tidyverse)
library(quartets)
library(knitr)
...
As the table above shows, all four of the different datasets show the same means, variances, and correlations (more on that in chapter 6). With *just* these summary statistics, you'd likely think "eh, all the data is the same, these datasets are basically identical."

**Wrong.**

When we graph these four datasets, we see something totally different than what the table shows.

```{r Anscombe-graph, fig.cap = "While the four datasets looked identical in the table, the visualized Anscombe datasets show an entirely different picture."}
...
## Learning Objectives {#viz-objectives}
...
```{r baseR-plot, fig.cap= "An example of a bar chart created using base R."}
...
```{r mpg-kable, message=FALSE, warning=FALSE}
...
### Scatterplot - geom_point() {#ggplot-scatterplot}
When data has two continuous (for example, numeric) variables, and you want to visualize their relationship, a scatterplot is a fantastic choice. This is the basis for relationship analysis that topics such as linear regression (more about this in Chapter \@ref(linear-reg)) rely on.

The geometry that needs to be specified is `geom_point()`.
```{r basic-scatterplot,fig.cap= "A basic example of a scatterplot using ggplot2."}
ggplot(data = mpg, aes(x = cty, y = hwy)) +
  geom_point()
```
...
```{r ggplot2-scatterplot-lm, fig.cap= "An example of a scatterplot with a line of best fit."}
# Line of Best Fit
...
```{r scatterplot-advanced, fig.cap= "A scatterplot incorporating multiple aesthetics and faceting."}
...
```{r barplot-coordflip, fig.cap= "An example of a coordinate flipped bar chart."}
...
```{r stacked-barchart-example, fig.cap= "An example of a stacked bar chart."}
...
```{r grouped-barchart, fig.cap= "An example of a grouped bar chart."}
...
```{r basic-columnchart, fig.cap= "An example of a basic column chart."}
...
```{r columnchart-aesthetics, fig.cap= "An example of a column chart with polished aesthetics."}
...
### Histograms and Density Plots (distribution) {#ggplot-histogram-density}
Histograms are perfect for when you are looking to display **distribution**.

#### Histograms - geom_histogram() {#ggplot-histogram}
For the geometry of a histogram, you use `geom_histogram()`.
```{r basic-histogram, fig.cap= "An example of a basic histogram."}
# BASIC
...
```{r styled-histogram,  fig.cap= "An example of a styled histogram."}
...
```{r filled-histogram, fig.cap= "An example of a filled histogram."}
...
#### Density - geom_density() {#ggplot-density}
...
```{r basic-densityplot, fig.cap= "An example of a basic density plot."}
# BASIC
...
```{r grouped-densityplot, fig.cap= "An example of a grouped density plot."}
# GROUPED
...
### Boxplot - geom_boxplot() {#ggplot-boxplot}
...
```{r basic-boxplot, fig.cap= "An example of a basic boxplot."}
# BASIC
...
```{r jittered-boxplot, fig.cap= "An example of a boxplot with jittered points."}
...
### Lines (time series) - geom_line() {#ggplot-line}
...
```{r basic-linegraph,  fig.cap= "An example of a basic line graph."}
...
```{r lm-linegraph, fig.cap= "An example of line graph with a LM line of best fit."}
...
```{r loess-linegraph, fig.cap= "An example of line graph with a LM line of best fit."}
ggplot(economics, aes(date, unemploy)) +
...
### Put text on the plot - geom_text() {#ggplot-text}
...
```{r basic-textplot, fig.cap= "An example of adding text inside a plot."}
...
```{r basic-error-plot, fig.cap= "An example of a plot with error bars."}
...
### Reference lines {#ggplot-ref-lines}
...
```{r horizontal-referencelines, fig.cap= "An example of plot with a horizontal reference line."}
...
```{r vertical-referencelines, fig.cap= "An example of plot with a vertical reference line."}
...
ggplot(mpg, aes(displ, hwy)) +
  ...
  labs(title = "Reference Line at Engine Displacement = 3L",
       x = "Engine Displacement (L)", y = "Highway MPG")
```
## Key Takeaways {#ggplot-takeaways}
...
## ggplot2 Visualization Reference {#ggplot-keyfunctions}
...
### Summary of ggplot Geometries {#ggplot-geometries}
...
### Summary of other ggplot commands {#ggplot-commands}
...
Within `ggplot2`, functions like `labs()` allow you to clearly label axes, titles, and legends so the intent of the plot is immediately clear. When working in R Markdown or bookdown (Section \@ref(r-markdown-graphs)), figure captions (using `fig.cap`) provide additional context that travels with the figure wherever it appears.
