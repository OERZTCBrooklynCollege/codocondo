...
However, that was when we had numeric data. What happens when we do not have any numeric data? From our last chapter, we know that is where a chi-square comes in, but what if we want to still do some predicting? This is exactly where **Logistic Regression** comes into play. The question we are trying to answer is:
...
## Learning Objectives {log-reg-objectives}
...
## Load and Preview Data {#log-reg-data}
...
```{r loading-data-ISLR2, message=FALSE, warning=FALSE}
library(ISLR2)
library(tidyverse)
...
*Note: the definitions come directly from the description found in the package*
...
## Exploratory Data Analysis {#log-reg-exploration}
...
```{r proportion-odds, message=FALSE, warning=FALSE}
...
## Visualize Relationships {#log-reg-viz}
...
```{r fig-default-bargraph, fig.cap="Most customers do not default on their credit cards, with defaulters representing only about 3% of the dataset."}
library(ggthemes)
library(ggpubr)
...
For a quick review on histograms, see Section \@ref(ggplot-histogram).

```{r fig-default-histogram, fig.cap="While most customers do not default on their credit cards, it seems that people that do have higher balances."}
library(ggthemes)
library(ggpubr)
...
```{r fig-default-scatter, message=FALSE, warning=FALSE, fig.cap="Regardless of if you default or not, there is a negative relationship between Annual Income and Average Credit Card Balance."}
# Balance vs. Income
...
## Train and Test Split {#log-reg-splitting-data}
...
## Build Logistic Regression Model {#log-reg-model}
When building a logistic regression model, it is going to look very similar to the linear regression model we built starting in Section \@ref(linear-reg-model). The major difference from a coding perspective between a linear regression and a logistic regression model is that instead of using `lm()`, you need to use `glm()`.

Before we create our model, let's take a step back. Logistic regression, just like linear regression, is a tool to help us predict an outcome. Specifically, we are predicting the **probability** of an outcome. The formula for logistic regression is as follows:
\[
\log\left(\frac{p}{1 - p}\right) = \beta_0 + \beta_1 \cdot \text{balance} + \beta_2 \cdot \text{income} + \beta_3 \cdot \text{student}
\]

where \(p\) is the probability of default. The left-hand side is called the *logit*, or log-odds. Notice that the right-hand side looks just like a straight-line regression. Now, let's get to business!

```{r creating-model, message=FALSE, warning=FALSE}
...
```{r convert-log-odds, message=FALSE, warning=FALSE}
...
### McFadden's Pseudo-R² {#mcfaddens-r-2}
Unlike in linear regression, logistic regression does not have an R²: (for a review on R²: see \@ref(linear-reg-model)). However, we can use the `pscl` library to calculate a pseudo R² value.

```{r McFadden-pseudo-R2, message=FALSE, warning=FALSE}
...
### Variable Importance {#var-importance}
...
Just like in Section \@ref(partial-cor), we want to check to see if there is any interactions between some of our variables that may be impacting our regression model. Using the `vif` function from the `car` library, we can see the interactions between our variables in our model.
...
## Interpretation {#log-reg-interpretation}
...
## Key Takeaways {#log-reg-takeaways}
...
## Key Functions & Commands {#log-reg-keyfunctions}
...
## Example APA-style Write-up {#log-reg-apa}
...
A logistic regression analysis was conducted to examine whether credit card balance, income, and student status predicted the likelihood of default. The overall logistic regression model was statistically significant, *χ*²(3) = 428.52, *p* < .001, and demonstrated good model fit, as indicated by McFadden’s pseudo-*R*² = .46. Higher credit card balances were associated with increased odds of default, whereas students had significantly lower odds of default than non-students. Income was not a significant predictor after accounting for balance and student status.

## 💡 *Reproducibility Tip:* {#log-reg-tip}
...
Using `set.seed()` (like in the reproducibility tip found in \@ref(anova-tip)) ensures that the same observations are assigned to the training and test sets each time the code is run. In addition, choices such as the train/test split ratio and classification threshold (e.g., 0.5) should be explicitly stated, as they directly affect model performance.
